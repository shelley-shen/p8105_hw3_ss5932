...
From this table of total activity per day, we cannot directly observe any trends aside from the fact that activity levels were much lower on the Saturday's of weeks 4 and 5. 

I also made a table aggregating the activity level per day of the week.
```{r}
accel_df %>% 
  group_by(day) %>% 
  summarize(total_activity = sum(activity_count)) %>% 
...
From this second table, we can observe that on average, the subject had the highest level of activity on Friday's and the lowest activity levels on Saturday.
